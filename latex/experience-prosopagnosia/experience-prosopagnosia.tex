...
\usepackage{enumitem}
\usepackage{amsmath}
% Classic BibTeX (alpha style approximates biblatex's alphabetic labels);
% biblatex has no LaTeXML binding, which blocks HTML generation.
...
\begin{document}
    ...
    \begin{quotation}
        \textit{"It is useless to base the defense of materialism on any analysis of mental phenomena that fails to deal explicitly with their subjective character" \nocite{nagel_1974} \hspace{1em plus 1fill}---Thomas Nagel}
    \end{quotation}
    ...
    \bibliography{experience-prosopagnosia}
\end{document}